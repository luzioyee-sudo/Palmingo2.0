...
function socialId(): string | null {
  return localStorage.getItem("palmingo:social-id");
}
...
export function getMyUsername(): string | null {
  return localStorage.getItem("palmingo:username");
}
...
export function searchUsers(q: string) {
  return apiFetch<SocialUser[]>(`/users/search?q=${encodeURIComponent(q)}`);
}

export function getUser(id: string) {
  return apiFetch<SocialUser>(`/users/${id}`);
}

/* ── Friends ───────────────────────────────────────── */
export function getFriends() {
  return apiFetch<Friendship[]>("/friends");
}

export function sendFriendRequest(addresseeId: string) {
  return apiFetch<Friendship>("/friends/request", {
    method: "POST", body: JSON.stringify({ addresseeId }),
  });
}

export function respondToFriend(friendshipId: string, accept: boolean) {
  return apiFetch<Friendship>("/friends/respond", {
    method: "POST", body: JSON.stringify({ friendshipId, accept }),
  });
}
...
export function leaveRoom(roomId: string) {
  return apiFetch<{ ok: boolean }>(`/rooms/${roomId}/leave`, { method: "POST" });
}

export function deleteRoom(roomId: string) {
  return apiFetch<{ ok: boolean }>(`/rooms/${roomId}`, { method: "DELETE" });
}
...
export function updateMember(roomId: string, memberId: string, opts: { role?: "moderator" | "member"; muted?: boolean }) {
  return apiFetch(`/rooms/${roomId}/members/${memberId}`, {
