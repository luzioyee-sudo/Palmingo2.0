...
export type RoomEvent =
  | { type: "registered" }
  | { type: "room_joined"; roomId: string; members: OnlineMember[]; targetLang: string }
  | { type: "member_online"; userId: string; username: string; role: string; muted: boolean; initiateOffer: boolean }
...
export interface OnlineMember {
  ...
  role: "admin" | "moderator" | "member"; muted: boolean; online: boolean;
}
...
export class PalmWsClient {
  ...
  sendChat(roomId: string, content: string) {
    this.send({ type: "chat", roomId, content });
  }

  muteUser(roomId: string, targetId: string, muted: boolean) {
    this.send({ type: "mute", roomId, targetId, muted });
  }

  kickUser(roomId: string, targetId: string) {
    this.send({ type: "kick", roomId, targetId });
  }

  setRole(roomId: string, targetId: string, role: "moderator" | "member") {
    this.send({ type: "set_role", roomId, targetId, role });
  }

  requestChallenge(roomId: string) {
    this.send({ type: "challenge", roomId });
  }

  /* ── WebRTC ──────────────────────────────────── */
  setOnRemoteStream(cb: (peerId: string, stream: MediaStream | null) => void) {
  ...
  private async createPeer(peerId: string, initiator: boolean) {
    ...
    pc.onicecandidate = (e) => {
      if (e.candidate) {
        this.send({ type: "rtc_ice", roomId: this.currentRoom!, toId: peerId, candidate: e.candidate });
      }
    };
  ...
  private async handleServerMsg(msg: Record<string, unknown>) {
    // Forward most events to handlers
    switch (msg.type) {
      case "registered":
      ...
      case "room_joined":
        this.emit({
          ...
          members: msg.members as OnlineMember[],
          targetLang: msg.targetLang as string,
        });
        break;

      case "member_online": {
        const ev = msg as { userId: string; username: string; role: string; muted: boolean; initiateOffer: boolean };
        this.emit({ type: "member_online", ...ev });
        // Initiate WebRTC offer to the new member (we're an existing member)
        if (ev.initiateOffer && this.localStream) {
          await this.createPeer(ev.userId, true);
        }
        break;
      }

      case "member_offline":
        this.emit({ type: "member_offline", userId: msg.userId as string });
        this.closePeer(msg.userId as string);
        break;

      case "member_muted":
        this.emit({ type: "member_muted", userId: msg.userId as string, muted: msg.muted as boolean });
        break;

      case "member_kicked":
        this.emit({ type: "member_kicked", userId: msg.userId as string });
        this.closePeer(msg.userId as string);
        break;

      case "role_changed":
        this.emit({ type: "role_changed", userId: msg.userId as string, role: msg.role as string });
        break;

      case "you_were_muted":
        this.emit({ type: "you_were_muted", muted: msg.muted as boolean });
        this.setMicEnabled(!(msg.muted as boolean));
        break;

      case "you_were_kicked":
        this.emit({ type: "you_were_kicked", roomId: msg.roomId as string });
        this.closeAllPeers();
        this.currentRoom = null;
        break;

      case "new_message":
        this.emit({ type: "new_message", message: msg.message as WsMessage });
        break;

      case "rtc_offer":
        await this.handleOffer(msg.fromId as string, msg.offer as RTCSessionDescriptionInit);
        break;

      case "rtc_answer":
        await this.handleAnswer(msg.fromId as string, msg.answer as RTCSessionDescriptionInit);
        break;

      case "rtc_ice":
        await this.handleIce(msg.fromId as string, msg.candidate as RTCIceCandidateInit);
        break;
